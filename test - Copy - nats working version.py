...
import xarray as xr
filelocation = r"C:\Users\super\PyCharmProjects\untitled\Nationals_Tab_Cards.pdf"
            ...
            if team1_fixed.iloc[0,b] is not team1_fixed.iloc[0,7]:
                ...
                if not alreadyExists:
                    ...
                    for round in range (0,numberOfRounds):
                        ...
                        newDatafile = newDatafile.append(pd.Series(["Nationals 2019",np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],index = newDatafile.columns), ignore_index = True)
                        newDatafile.iloc[round, 1] = team1_fixed.iloc[round+2,0] #Round
                        newDatafile.iloc[round, 2] = team1_fixed.iloc[round+2, 1] # G/O
                        newDatafile.iloc[round, 3] = team1_fixed.iloc[round+2, 2] # W/L
                        newDatafile.iloc[round, 4] = team1_fixed.iloc[round+2,3] # Opponent
                        newDatafile.iloc[round, 5] = team1_fixed.iloc[round+2,4] # Judge
                        ...
                        for namelook in range(0,tab_card_size[1]):
                            ...
                            if ownNameCheck == name:
                                ...
                                if yourNumber < partnerKey:
                                    speaks_ranks = team1_fixed.iloc[round + 2, -3]
                                if yourNumber > partnerKey:
                                    speaks_ranks = team1_fixed.iloc[round + 2, -2]
